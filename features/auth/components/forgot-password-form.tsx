...
export function ForgotPasswordForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message = getApiErrorMessage(submitError, t.forgotPasswordFailed);
      setError(message);
      toast.error(message);
    } finally {
      setIsPending(false);
    ...
    <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
      <FieldGroup>
        ...
        <Field>
          ...
          />
          <FieldDescription>{t.forgotPasswordEmailHelp}</FieldDescription>
        </Field>
        {error ? (
        ...
        <Field>
          <Button type="submit" disabled={isPending}>
            {isPending ? t.sendResetLinkPending : t.sendResetLink}
          </Button>
